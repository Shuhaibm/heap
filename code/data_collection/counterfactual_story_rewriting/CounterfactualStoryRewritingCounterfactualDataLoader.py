...
# Task 5: Which rewritten consideration is more reasonable given the Premise and COUNTERFACTUAL sentence provided? (answer.counterfactual)

import csv
from data_collection.DataLoader import DataLoader

class CounterfactualStoryRewritingCounterfactualDataLoader(DataLoader):
        ...
        for human_eval_results_file in human_eval_results_files:
            with open(human_eval_results_file) as csvfile:
                ...
                for i, row_arr in enumerate(reader):
                    ...
                    if row["Answer.counterfactual"] == "A":
                        data.append({
                    ...
                    if row["Answer.counterfactual"] == "B":
                        data.append({
                            "premise":row["Input.X1"],"initial_second":row["Input.X2"],"original_end":row["Input.Y"],
                            "counterfactual":row["Input.XX2"],
