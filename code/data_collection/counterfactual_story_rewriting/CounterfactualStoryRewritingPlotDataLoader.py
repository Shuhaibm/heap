...
# Task 3: Select the plot presented in the rewritten continuation that is more related to the plot of the original sentence (answer.plot)

import csv
from data_collection.DataLoader import DataLoader

class CounterfactualStoryRewritingPlotDataLoader(DataLoader):
    ...
    def concatenate_data(self):
        ...
        for data_point in data:
            ...
            if self.include_tags:
                good_sample = "story" + self.sep_token + premise + counterfactual + self.sep_token +"original ending" + self.sep_token + original_end + self.sep_token + "rewritten ending" + self.sep_token + "ending" + self.sep_token + better_rewritten
                bad_sample = "story" + self.sep_token + premise + counterfactual + self.sep_token +"original ending" + self.sep_token + original_end + self.sep_token + "rewritten ending" + self.sep_token + "ending" + self.sep_token + worse_rewritten
            else:
                good_sample = premise + counterfactual + self.sep_token + original_end + self.sep_token + better_rewritten
                bad_sample = premise + counterfactual + self.sep_token + original_end + self.sep_token + worse_rewritten
